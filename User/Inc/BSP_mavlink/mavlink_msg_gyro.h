 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gyro_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               int16_t gx, int16_t gy, int16_t gz, int16_t ax, int16_t ay, int16_t az)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_GYRO_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_GYRO_MIN_LEN, MAVLINK_MSG_ID_GYRO_LEN, MAVLINK_MSG_ID_GYRO_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_GYRO_MIN_LEN, MAVLINK_MSG_ID_GYRO_LEN);
#endif
}
...
/**
 * @brief Encode a gyro struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param gyro C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_gyro_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_gyro_t* gyro)
{
    return mavlink_msg_gyro_pack_status(system_id, component_id, _status, msg,  gyro->gx, gyro->gy, gyro->gz, gyro->ax, gyro->ay, gyro->az);
}
